...
const SUGGEST = ["この本の中心概念を一番簡単に説明すると？", "第1章のポイントは？", "この考えを自分の仕事でどう使える？", "著者の主張の弱点は？"];
...
export default function Chat({ bookId, ready }: { bookId: string; ready: boolean }) {
  ...
  return (
    <section id="ask" className="card p-4 sm:p-6">
      <div className="flex items-center justify-between mb-3">
        <h2 className="font-semibold text-lg">この本について質問する</h2>
      ...
      {ready && msgs.length === 0 && (
        <div className="flex flex-wrap gap-2 mb-3">
          {SUGGEST.map((s) => (
            <button key={s} onClick={() => send(s)} className="text-xs rounded-full border border-line px-3 py-1.5 hover:border-accent">{s}</button>
          ))}
        </div>
      )}
      ...
      >
        <input
          ...
          disabled={!ready || busy}
          placeholder="例：この本でいうResultingって何？"
